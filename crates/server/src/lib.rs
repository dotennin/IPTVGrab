...
struct AppState {
    ...
    transcodes: Arc<RwLock<HashMap<String, Arc<TranscodeSession>>>>,
...
fn build_state(
    ...
    AppState {
        ...
        transcodes: Arc::new(RwLock::new(HashMap::new())),
...
// ── FLV→HLS transcode handlers ────────────────────────────────────────────────

#[derive(Deserialize)]
struct StartTranscodeBody {
    url: String,
}
...
async fn start_transcode(
    State(state): State<AppState>,
    Json(body): Json<StartTranscodeBody>,
) -> impl IntoResponse {
    if !body.url.starts_with("http://") && !body.url.starts_with("https://") {
        return (StatusCode::BAD_REQUEST, Json(serde_json::json!({"detail": "Invalid URL"}))).into_response();
    }

    // Reuse an existing live session for this source URL.
    {
    ...
    if let Err(e) = tokio::fs::create_dir_all(&tmp_dir).await {
        return (StatusCode::INTERNAL_SERVER_ERROR,
            Json(serde_json::json!({"detail": format!("tmpdir: {e}")}))).into_response();
    }
    ...
    // FFmpeg reads the FLV URL directly — ONE CDN connection, satisfying the
    // per-IP concurrent-connection limit enforced by Huya/Tengine live CDN.
    // -c:v copy   — remux video as-is (no transcode cost)
    // -c:a aac    — ensure audio is AAC (required for HLS on iOS; FLV may carry MP3)
    // -hls_flags delete_segments+append_list — rolling window for live streams
    ...
    let child = match child {
        ...
        Err(e) => {
            ...
            return (StatusCode::INTERNAL_SERVER_ERROR,
                Json(serde_json::json!({"detail": format!("ffmpeg spawn: {e}")}))).into_response();
        }
    };
    ...
    state.transcodes.write().await.insert(body.url.clone(), session.clone());
...
///
/// Reads the FFmpeg-written playlist from disk and rewrites segment filenames
/// to `/api/watch/transcode/:id/<seg>` so the browser fetches through us.
async fn transcode_playlist(
    State(state): State<AppState>,
    AxumPath(id): AxumPath<String>,
) -> impl IntoResponse {
    // Find session by id (value lookup — sessions are keyed by source_url).
    let session = {
        let map = state.transcodes.read().await;
    ...
    let content = match tokio::fs::read_to_string(&m3u8_path).await {
        Ok(c) => c,
        Err(_) => {
            // FFmpeg hasn't written the file yet — return an empty live playlist.
            return Response::builder()
                .status(StatusCode::OK)
    ...
    // Rewrite relative segment paths: `seg_00001.ts` → `/api/watch/transcode/{id}/seg_00001.ts`
    let rewritten: String = content
        .lines()
        .map(|line| {
            let t = line.trim();
            if !t.is_empty() && !t.starts_with('#') && t.ends_with(".ts") {
                // Strip any directory prefix FFmpeg may have written.
